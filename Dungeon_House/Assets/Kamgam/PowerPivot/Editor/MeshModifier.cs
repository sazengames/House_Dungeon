...
namespace Kamgam.PowerPivot
{
    public partial class MeshModifier
    {
        ...
        /// 
        /// </summary>
        /// <param name="pivotDeltaPos">How much the pivot should be moved.</param>
        /// <param name="pivotDeltaRot">How much the pivot should be rotated.</param>
        /// <param name="updateColliders">Whether or not the colliders should be displaced too.</param>
        public Mesh CopyOrUpdateMesh(Vector3 pivotDeltaPos, Quaternion pivotDeltaRot, bool updateColliders)
        {
            ...
            if (currentPath.EndsWith(FILE_EXTENSION))
            {
                ...
                {
                    displaceVertices(existingMesh, existingMesh, pivotDeltaPos, pivotDeltaRot);
                ...
                if (IsSkinned)
                {
                    ...
                    bounds.center -= pivotDeltaPos;
                    SkinnedMeshRenderer.bounds = bounds;
                }

                if (updateColliders)
                    displaceColliders(GameObject, pivotDeltaPos, pivotDeltaRot, existingMesh);

                // Update pivot data if it exists
                PivotData pivotData = LoadPivotDataFromNewMeshPath(currentPath);
                if(pivotData != null)
                {
                    pivotData.PivotDeltaPos += pivotDeltaPos;
                ...
                else
                {
                    ...
                    {
                        pivotData = PivotData.CreateOrUpdate(pivotData, originalMesh, existingMesh, pivotDeltaPos, pivotDeltaRot);
            ...
            else
            {
                ...
                pivotData = PivotData.CreateOrUpdate(pivotData, currentMesh, newMesh, pivotDeltaPos, pivotDeltaRot);
                ...
                if (newMesh == null)
                {
                    ...
                    CopyAndDisplaceMesh(currentMesh, newMesh, pivotDeltaPos, pivotDeltaRot);
                ...
                if (updateColliders)
                    displaceColliders(GameObject, pivotDeltaPos, pivotDeltaRot, newMesh);

                return newMesh;
            }
        }

        public static void CopyAndDisplaceMesh(Mesh source, Mesh target, Vector3 pivotDeltaPos, Quaternion pivotDeltaRot)
        {
            target.Clear();
            ...
            displaceVertices(source, target, pivotDeltaPos, pivotDeltaRot);

            target.RecalculateBounds();
        }

        static void displaceVertices(Mesh source, Mesh target, Vector3 pivotDeltaPos, Quaternion pivotDeltaRot)
        {
            var vertices = source.vertices;
            int vCount = vertices.Length;
            float dX = -pivotDeltaPos.x;
            float dY = -pivotDeltaPos.y;
            float dZ = -pivotDeltaPos.z;
            Quaternion invRot = Quaternion.Inverse(pivotDeltaRot);
        ...
        protected void displaceColliders(GameObject go, Vector3 pivotDeltaPos, Quaternion pivotDeltaRot, Mesh updatedMesh)
        {
            ...
            {
                // Old behaviour was to always update the mesh collider but usually that is the already changed
                // mesh so now it's an option in the settings and OFF by default.
                if (settings.ChangeMeshColliderGeometry)
                {
                    displaceVertices(meshCollider.sharedMesh, meshCollider.sharedMesh, pivotDeltaPos, pivotDeltaRot);
                }
                else
                {
            ...
            foreach (var boxCollider2D in boxColliders2D)
            {
                ...
                offset.x -= pivotDeltaPos.x;
                offset.y -= pivotDeltaPos.y;
            ...
            foreach (var polygonCollider2D in polygonColliders2D)
            {
                ...
                {
                    polygonCollider2D.points[i].x -= pivotDeltaPos.x;
                    polygonCollider2D.points[i].y -= pivotDeltaPos.y;
                }
            }
        }
        ...
        public void ResetAll()
        {
            ...
            {
                CopyAndDisplaceMesh(_meshCopy, GetSharedMeshFromComponent(), Vector3.zero, Quaternion.identity);
