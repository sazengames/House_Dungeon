...
namespace Kamgam.PowerPivot
{
    ...
    partial class PowerPivotTool : EditorTool, ISerializationCallbackReceiver
    {
        ...
        {
            // Do not activate if use is currently using the spline tool.
#if KAMGAM_UNITY_SPLINES
            var toolName = ToolManager.activeToolType.Name;
            if (toolName.EndsWith("SplineMoveTool") || toolName.EndsWith("SplineScaleTool") ||
                toolName.EndsWith("SplineRotateTool"))
            {
                return;
            }
#endif
        ...
        public void OnToolChanged()
        {
            ...
            {
                if (!EditorApplication.isPlayingOrWillChangePlaymode && SceneView.lastActiveSceneView != null)
                    SceneView.lastActiveSceneView.Focus();
                ...
                RegisterCursorUndo(cursorPosition, cursorRotation);
                mouseIsDown = false;
            }
            else
            {
