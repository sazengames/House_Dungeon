...
namespace Kamgam.PowerPivot
{
    ...
    partial class PowerPivotTool
    {
        ...
        void drawWindowContent(int controlID)
        {
            ...
            Vector3 localCursorRot = isLocalCursor ? targetTransform.InverseTransformRotation(cursorRotation).eulerAngles : cursorRotation.eulerAngles;
            TrimVectorForDisplay(ref localCursorRot);
            ...
            if (currentTool == PTool.Cursor)
            {
                ...
                // Pos
                string name = foldoutChangePivot ? "Pivot" : "Cursor";
                var newLocalCursorPos = EditorGUILayout.Vector3Field(name + " Pos.: ", localCursorPos);
                ...
                if (Vector3.Magnitude(delta) > 0.0001f)
                {
                    ...
                    RegisterCursorUndo(cursorPosition, cursorRotation, minTimeDelta: 2);
                }
                
                drawPositionShortcutButtons();

                if (foldoutChangePivot)
                {
                    // Rot
                    GUI.enabled = isLocalCursor;
                    var newLocalCursorRot = EditorGUILayout.Vector3Field("Pivot Rot.:", localCursorRot);
                    var rotDelta = newLocalCursorRot - localCursorRot;
                    // Update cursor rotation (though we only do it so the handle updates, the change is undone automatically if the users switches to a non editing mode).
                    // See "Maybe in a future update" comments integrating rotation into the regular workflow would take more time and I don't think it's worth it in terms of work flow (makes things more complicated).
                    if (Vector3.Magnitude(rotDelta) > 0.0001f)
                    {
                        if (isLocalCursor)
                            cursorRotation = targetTransform.TransformRotation(Quaternion.Euler(newLocalCursorRot));
                        else
                        {
                            rotDelta = cursorRotation * rotDelta;
                            cursorRotation = Quaternion.Euler(rotDelta);
                        }
            ...
            if (currentTool == PTool.Cursor)
            {
                ...
                if (foldoutChangePivot)
                {
                    ...
                    if (GUILayout.Button(new GUIContent("Save", "Copies the mesh, moves the vertices and then saves and assigns it.")))
                    {
                        if (gameObject != null)
                        {
                            var pivotDeltaPos = gameObject.transform.InverseTransformPoint(cursorPosition);
                            var pivotDeltaRot = gameObject.transform.InverseTransformRotation(cursorRotation);
                            ...
                            var newMesh = modifier.CopyOrUpdateMesh(pivotDeltaPos, pivotDeltaRot, updateColliders);
                            modifier.AssignSharedMesh(newMesh);
                            PrefabUtility.RecordPrefabInstancePropertyModifications(modifier.Component);

                            // Move the cursor back to local zero pos and rotation
                            cursorPosition = gameObject.transform.TransformPoint(Vector3.zero);
                            cursorRotation = gameObject.transform.rotation;

                            if (!modifier.IsSkinned || !modifier.HasBones)
                            {
                                // Move object by pivot delta pos so it stays in place.
                                var posDeltaInWorld = gameObject.transform.TransformVector(pivotDeltaPos);
                                var pos = gameObject.transform.position;
                                pos += posDeltaInWorld;
                                gameObject.transform.position = pos;
                                
                                // Rotate object by pivot delta rot so it stays in place.
                                var newRot = gameObject.transform.localRotation * pivotDeltaRot;
        ...
        private void drawPositionShortcutButtons()
        {
            // Pos shortcuts
            GUILayout.BeginHorizontal();
            GUILayout.Space(30);
            bool isLocal = Tools.pivotRotation == PivotRotation.Local;
            if (GUILayout.Button(new GUIContent("P", "Moves the pivot position back to the original pivot.")))
            {
                cursorPosition = calculateDefaultHandlePivotPosition();
            }
            if (GUILayout.Button(new GUIContent("BC", "Moves the pivot position to bottom center.")))
            {
                cursorPosition = calculatePivotPositionBoundBoxPosition(cursorPosition, new Vector3(0f, -1f, 0f), isLocal);
            }

            if (GUILayout.Button(new GUIContent("TC", "Moves the pivot position to top center.")))
            {
                cursorPosition = calculatePivotPositionBoundBoxPosition(cursorPosition, new Vector3(0f, 1f, 0f), isLocal);
            }

            if (GUILayout.Button(new GUIContent("LC", "Moves the pivot position to left center.")))
            {
                cursorPosition = calculatePivotPositionBoundBoxPosition(cursorPosition, new Vector3(-1f, 0f, 0f), isLocal);
            }

            if (GUILayout.Button(new GUIContent("RC", "Moves the pivot position to right center.")))
            {
                cursorPosition = calculatePivotPositionBoundBoxPosition(cursorPosition, new Vector3(1f, 0f, 0f), isLocal);
            }

            if (GUILayout.Button(new GUIContent("BC", "Moves the pivot position to back center.")))
            {
                cursorPosition = calculatePivotPositionBoundBoxPosition(cursorPosition, new Vector3(0f, 0f, -1f), isLocal);
            }

            if (GUILayout.Button(new GUIContent("FC", "Moves the pivot position to front center.")))
            {
                cursorPosition = calculatePivotPositionBoundBoxPosition(cursorPosition, new Vector3(0f, 0f, 1f), isLocal);
            }
