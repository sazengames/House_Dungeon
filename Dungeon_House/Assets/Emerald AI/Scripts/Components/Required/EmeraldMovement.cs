...
namespace EmeraldAI
{
    ...
    public class EmeraldMovement : MonoBehaviour
    {
        ...
        bool ApplyPartialTargetPath;
        Vector3 CombatTargetNavMeshPosition;

        public float OffNavMeshDestinationUpdateSeconds = 0.5f;
        float OffNavMeshDestinationUpdateTimer;
        #endregion
        ...
        public void InitializeMovement ()
        {
            ...
            CalculatePathTimer = Random.Range(0f, CalculatePathSeconds);
            SetupNavMeshAgent();
        ...
        public void CombatMovement()
        {
            ...
            if (!AnimationComponent.IsBackingUp && !AnimationComponent.IsEquipping && !AnimationComponent.IsSwitchingWeapons && !EmeraldComponent.CombatComponent.DeathDelayActive && CanReachTarget && EmeraldComponent.m_NavMeshAgent.isOnNavMesh)
            {
                ...
                else if (EmeraldComponent.CoverComponent.CoverState == EmeraldCover.CoverStates.Inactive && EmeraldComponent.CombatComponent.DistanceFromTarget > EmeraldComponent.CombatComponent.CurrentAttackData.AttackDistance)
                {
                    m_NavMeshAgent.destination = EmeraldComponent.CombatTarget.position;
                }
            }

            if (!CanReachTarget && EmeraldComponent.CombatTarget != null && m_NavMeshAgent.isOnNavMesh)
            {
                OffNavMeshDestinationUpdateTimer += Time.deltaTime;

                if (OffNavMeshDestinationUpdateTimer >= OffNavMeshDestinationUpdateSeconds)
                {
                    OffNavMeshDestinationUpdateTimer = 0f;

                    if (TryGetCombatTargetNavMeshPosition(out Vector3 Position)) m_NavMeshAgent.destination = Position;
                    else m_NavMeshAgent.destination = transform.position;
                }
            }
            else
            {
        ...
        public bool CanReachTargetInternal()
        {
            ...
            {
                ApplyPartialTargetPath = false;
                return false;
            }

            CalculatePathTimer += Time.deltaTime;

            if (CalculatePathTimer >= CalculatePathSeconds)
            {
                CalculatePathTimer = 0f;

                bool TargetIsOnNavMesh = NavMesh.SamplePosition(EmeraldComponent.CombatTarget.position, out NavMeshHit Hit, 1.25f, m_NavMeshAgent.areaMask);

                if (TargetIsOnNavMesh) CombatTargetNavMeshPosition = Hit.position;

                bool TargetPositionFound = TargetIsOnNavMesh || TryGetCombatTargetNavMeshPosition(out CombatTargetNavMeshPosition);
                bool PathFound = TargetPositionFound && m_NavMeshAgent.CalculatePath(CombatTargetNavMeshPosition, AIPath);

                CanReachTarget = TargetIsOnNavMesh && PathFound && AIPath.status == NavMeshPathStatus.PathComplete;
                ApplyPartialTargetPath = PathFound && !CanReachTarget;
            }

            return CanReachTarget;
        }

        bool TryGetCombatTargetNavMeshPosition(out Vector3 Position)
        {
            Vector3 TargetPosition = EmeraldComponent.CombatTarget.position;
            float SampleDistance = Mathf.Max(2f, m_NavMeshAgent.height * 2f);

            if (NavMesh.SamplePosition(TargetPosition, out NavMeshHit Hit, SampleDistance, m_NavMeshAgent.areaMask))
            {
            ...
            if (NavMesh.SamplePosition(TargetPosition, out Hit, SampleDistance, m_NavMeshAgent.areaMask))
            {
                Position = Hit.position;
                return true;
            }
